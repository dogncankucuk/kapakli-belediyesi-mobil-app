...
const emptyForm: OnemliKurumInput = { ad: '', tur: 'Belediye', adres: '', lat: 41.33, lng: 27.97 };

function OnemliKurumlarPage({ canManage }: Props) {
  const [items, setItems] = useState<OnemliKurum[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState<OnemliKurumInput>(emptyForm);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<OnemliKurumInput>(emptyForm);
  ...
  async function load() {
    ...
    setError(null);
    try {
      setItems(await getOnemliKurumlar());
    } catch (err) {
  ...
  function startEdit(item: OnemliKurum) {
  ...
  async function handleUpdate(id: string) {
    setError(null);
    try {
      await updateOnemliKurum(id, editForm);
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Bu kurum kaydı silinsin mi?')) return;
    setError(null);
    try {
      await deleteOnemliKurum(id);
      await load();
    } catch (err) {
        ...
        <form className="inline-form" onSubmit={handleCreate}>
          ...
          <label>
            ...
            <select value={form.tur} onChange={(e) => setForm({ ...form, tur: e.target.value })}>
              {Object.entries(kurumTuruLabels).map(([value, label]) => (
          ...
          <tbody>
            {items.map((item) =>
              editingId === item.id ? (
                ...
                <tr key={item.id}>
                  <td>{item.ad}</td>
                  <td>{kurumTuruLabels[item.tur as keyof typeof kurumTuruLabels] ?? item.tur}</td>
                  <td>{item.adres ?? '-'}</td>
                  {canManage && (
...
export default OnemliKurumlarPage;
